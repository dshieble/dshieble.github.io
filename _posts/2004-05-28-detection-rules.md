...
The performance of this prompt will likely by bottlenecked by how well the payment features are described to the model. Critically, any information about the payment that a human labeler can access should be included in the prompt to achieve the best performance. More complex solutions to Task 1 might involve finetuning the LLM on labeled data or breaking the problem into subproblems handled by separate agents. 
