...
It turns out that the formal derivative in $$t$$ of $$\mathcal{B}$$ is the continuous-time white noise process. It isn't too hard to see why this should be the case: by the conditions above, the random variables formed from the increments in Brownian motion are independent and normally distributed. The differentiation process just continuous-ifies this. This suggests that we could reasonably hand-wave white noise to be the derivative in $$t$$ of the Brownian motion process. Of course, things are more complex than this. In fact, for almost every $$\omega \in [0,1]$$ the function $$\mathcal{B}_t(\omega)$$ is nowhere continuous in $$t$$. 
