...
However, there are circumstances under which it can be useful to use clustering for supervised learning. That is, given a collection of partitioned training datasets we would like to learn a strategy to partition new datasets in as similar a way as possible. Said another way, given a collection of tuples $$\mathbf{S} = \{(X, d_X, P_X)\}$$ where $$P_X$$ is a partition of $$X$$ we would like to learn a function $$f: (X, d_X) \rightarrow (X, P_X)$$ such that for each $$(X_i, d_{X_i}) \in \mathbf{S}$$ we have $$f(X_i, d_{X_i}) = (X_i, P_{X_i})$$. 
...
![Kan Extension](/img/kan_extension_pic)
