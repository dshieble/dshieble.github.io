...
* $$\delta_{LanK}$$ must be the smallest it can be such that the output of the $$\delta$$-single linkage functor is always refined by the output of the $$(\delta, \sigma)$$-single linkage functor, which is $$max(\delta, \sup \{\mu_{X}^{\sigma}(x) | x \in X, (X, d_X) \in \mathbf{T}\})$$.
<!-- 
