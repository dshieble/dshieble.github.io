...
* $$\delta_{RanK}$$ must be the largest it can be such that the output of the $$\delta$$-single linkage functor refines the output of the $$(\delta, k)$$-single linkage functor, which is $$\delta$$.
* $$\delta_{LanK}$$ must be the smallest it can be such that the output of the $$\delta$$-single linkage functor is always refined by the output of the $$(\delta, k)$$-single linkage functor, which is $$max(\delta, \sup \{\mu_{X_k}(x) \mid x \in X, (X, d_X) \in \mathbf{T}\})$$.
