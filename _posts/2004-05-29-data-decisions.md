...
It's often infeasible to get an answer to the maximally specified question. For example, perhaps the only dataset of user ad impressions that is available does not stratify by display location. However, simply understanding the gap between the question that we have an answer to (are users more likely to churn if they see more than 3 ads on their first day) and the question that we want an answer to (are users more likely to churn if they see more than 3 front page ads on their first day) helps us correctly calibrate our decision making confidence
