...
It is clear that the Kan extension can be a useful tool for deriving a clustering algorithm from labeled data.

For example, if different groups of human labellers have hand-categorized many sets of items based on their internal similarities (as opposed to pre-defined categories), we can use the Kan extensions to pick out the underlying clustering structure 
